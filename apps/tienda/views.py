...
class PuntoVentaPOSView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Filtro de tiempo para "Hoy" (Hora Colombia)
        hoy = timezone.now().date()
        
        # Consultamos las ventas del día de este tenant
        ventas_hoy = VentaTienda.objects.filter(academia=self.request.tenant, fecha__date=hoy)
        
        # Métricas agregadas de BD (Súper eficiente)
        resumen = ventas_hoy.aggregate(
            total_diario=Sum('total_venta'),
            ganancia_diaria=Sum('total_ganancia')
        )
        ...
        # Enviamos las métricas al template
        context['total_diario'] = resumen['total_diario'] or 0
        context['ganancia_diaria'] = resumen['ganancia_diaria'] or 0
...
class PanelInventarioView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return Producto.objects.filter(academia=self.request.tenant).order_by('stock')
    ...
    def get_context_data(self, **kwargs):
        ...
        # --- NUEVO: Métrica de Inversión / Compras del DÍA (Hora Colombia) ---
        tz_col = zoneinfo.ZoneInfo('America/Bogota')
        now_col = timezone.now().astimezone(tz_col)
        
        # Filtramos las ENTRADAS de stock (compras) hechas exactamente hoy
...
class ReporteVentasDiaView(LoginRequiredMixin, View):
    def get(self, request, slug_academia):
        ...
        # 1. Filtramos las ventas del día
        ventas = VentaTienda.objects.filter(academia=request.tenant, fecha__date=fecha)
        
        # 2. Agrupamos los detalles de venta (esto evita filas repetidas)
        # Sumamos cantidad, subtotal y ganancia agrupando por nombre de producto
        ...
        # 3. Totales generales para el header del reporte
        ...
        return JsonResponse({
            ...
            'detalles': list(detalle_resumen) # Enviamos la lista de productos
        })